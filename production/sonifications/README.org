...
The connections between the nodes are decorated with one sine wave oscillator each that together sum up as additive textures of spring sounds. The volume and pitch of these connections is influenced by the velocity of their change of length. Stereo panorama is mapped according to their horizontal position on screen.
